...
class Spoofer:
    ...
    def prompt_mouse_selection(self):
        ...
        if not detected_mice:
            print(Fore.YELLOW + "[!] No mouse device found.\nExiting in 10 seconds...")
            time.sleep(10)
            exit()
        os.system('cls')
        valid_mice = {}
        ...
        if not valid_mice:
            print(Fore.YELLOW + "[!] No valid USB Input Device found.\nExiting in 10 seconds...")
            time.sleep(10)
            exit()
    ...
    def install_avr_core(self):
        ...
        if "arduino:avr" not in result.stdout and not "1.8.6" in result.stdout:
            print(Fore.GREEN + "[+] Installing AVR core 1.8.6...")
        ...
        if "Mouse" not in result.stdout:
            print(Fore.GREEN + "[+] Installing Mouse library...")
    ...
    def compile_sketch(self):
        ...
        if not os.path.exists(self.SKETCH_FILE):
            print(Fore.RED + f"[!] Error: Sketch file '{self.SKETCH_FILE}' not found!")
            return
    ...
    def upload_sketch(self, com_port):
        ...
        if exit_code == 0:
            print(Fore.GREEN + "[+] Spoof finished successfully, you can now use the colorbot!")
        else:
            print(Fore.RED + f"[!] Failed to upload sketch. Error code: {exit_code}")
            print(Fore.RED + "[!] Make sure the Arduino is connected and the COM port is correct.")
